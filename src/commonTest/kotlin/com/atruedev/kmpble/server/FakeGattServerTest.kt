...
class FakeGattServerTest {
    ...
    @Test
    fun notify_records_are_captured() =
        runTest {
    ...
    @Test
    fun indicate_records_are_captured() =
        runTest {
            val server = FakeGattServer()
            server.open()
            server.simulateConnection(device1)
    ...
    @Test
    fun simulateConnection_updates_connections_flow() =
        runTest {
            val server = FakeGattServer()
            server.open()
    ...
    @Test
    fun simulateDisconnection_removes_from_connections() =
        runTest {
            val server = FakeGattServer()
            server.open()
    ...
    @Test
    fun notify_with_null_device_is_captured() =
        runTest {
            val server = FakeGattServer()
            server.open()
    ...
    @Test
    fun multiple_notify_calls_are_captured_in_order() =
        runTest {
            val server = FakeGattServer()
            server.open()
            server.simulateConnection(device1)
