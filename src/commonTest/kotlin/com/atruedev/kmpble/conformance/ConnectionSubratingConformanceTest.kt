...
public abstract class ConnectionSubratingConformanceTest : BleConformanceTest() {
    ...
    fun `multiple subrating requests can be issued after reconnect`() =
        runTest {
            ...
            // Disconnect and reconnect should allow subrating again
            peripheral.disconnect()
            peripheral.connect(ConnectionOptions())
    ...
    fun `subrating does not interfere with GATT operations`() =
        runTest {
            ...
            // Read after subrating still works
    ...
    @Test
    fun `subrating parameters are immutable data class`() =
        runTest {
            val peripheral =
                buildPeripheral {
                    service("180d") {
            ...
            // Verify the returned parameters are a copy (data class equality, not identity)
            assertEquals(params, result.parameters)
