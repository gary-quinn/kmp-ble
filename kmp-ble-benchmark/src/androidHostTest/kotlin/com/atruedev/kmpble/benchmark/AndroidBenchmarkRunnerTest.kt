...
/**
 * Android benchmark runner (host-side test).
 *
 * Executes the full [BleBenchmark] API on Android JVM host tests
 * to validate benchmark correctness and result consistency.
 *
 * Run: ./gradlew :kmp-ble-benchmark:androidHostTest --tests "*AndroidBenchmarkRunner*"
 */
...
class AndroidBenchmarkRunnerTest {
    ...
    @Test
    fun `full benchmark suite completes on Android host`() =
        runTest {
            val peripheral =
    ...
    @Test
    fun `latency tracker computes correct statistics on Android host`() =
        runTest {
            val tracker = LatencyTracker()
            tracker.measure { /* no-op */ }
            tracker.measure { /* no-op */ }

            val stats = tracker.summarize("android-latency")
            assertEquals(2, stats.count)
            assertTrue(stats.min.inWholeNanoseconds >= 0)
        }

    @Test
    fun `throughput meter accumulates correctly on Android host`() {
        val meter = ThroughputMeter()
        meter.start()
        meter.record(1024)
        meter.record(2048)
        val result = meter.stop("android-throughput")
