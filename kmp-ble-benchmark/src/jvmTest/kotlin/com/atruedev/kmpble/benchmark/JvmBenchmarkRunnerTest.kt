 ...
 *
 * Executes the full [BleBenchmark] API on JVM to validate
 * benchmark correctness and result consistency.
 *
 * Run: ./gradlew :kmp-ble-benchmark:jvmTest --tests "*JvmBenchmarkRunner*"
 */
@OptIn(ExperimentalBleApi::class)
class JvmBenchmarkRunnerTest {
    private val benchmark = BleBenchmark()

    @Test
    fun `full benchmark suite completes on JVM`() =
        runTest {
            val peripheral =
    ...
    @Test
    fun `latency tracker computes correct statistics on JVM`() =
        runTest {
            val tracker = LatencyTracker()
            tracker.measure { /* no-op */ }
            tracker.measure { /* no-op */ }

            val stats = tracker.summarize("jvm-latency")
            assertEquals(2, stats.count)
            assertTrue(stats.min.inWholeNanoseconds >= 0)
        }

    @Test
    fun `throughput meter accumulates correctly on JVM`() {
        val meter = ThroughputMeter()
        meter.start()
        meter.record(1024)
        meter.record(2048)
        val result = meter.stop("jvm-throughput")
