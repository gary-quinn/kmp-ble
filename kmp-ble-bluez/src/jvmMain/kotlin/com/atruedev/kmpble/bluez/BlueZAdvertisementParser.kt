...
/**
 * Maps BlueZ device properties to a backend [ScanRecord].
 */
@OptIn(ExperimentalUuidApi::class)
internal fun BlueZDeviceSnapshot.toScanRecord(): ScanRecord {
    val signalStrength = checkNotNull(rssi) { "BlueZDeviceSnapshot.rssi is required for ScanRecord mapping" }
    return ScanRecord(
        handle = dbusPath,
        ...
        serviceData =
            serviceData
                .mapNotNull { (uuid, bytes) ->
                    runCatching { Uuid.parse(uuid) to bytes }.getOrNull()
                }.toMap(),
